...
signed main()
{
	...
	for(int len=2;len<=n;len++)//枚举区间长度,1的话没有意义从2开始
	{
		...
		{
			int r=l+len-1;//终点
			for(int k=p[l][r-1];k<=p[l+1][r];k++)//枚举决策
			{
				if(g[l][r]>g[l][k]+g[k+1][r]+s[r]-s[l-1])//取最小的时候可以用p这个
				{
					g[l][r]=min(g[l][r],g[l][k]+g[k+1][r]+s[r]-s[l-1]);//两边大的合并就是左边的费用+右边的费用再加上左右两边总和
					p[l][r]=k;
				}
			}
			//取最大从两边找
