...
int f[maxn][maxn];//f[i][j]表示i位数最高位为j的个数
void init()
{
	for(int i=0;i<maxn;i++) f[1][i]=1;
	for(int i=2;i<maxn;i++) //阶段：枚举位数 
	{
		for(int j=0;j<=9;j++) //状态：枚举最高位 虽然不能有前导0但是要从0开始因为后面要用到
		{
...
int dp(int n)
{
	...
	int res=0,last=-2;        //last表示上一位的数字
	for(int i=cnt;i>=1;i--)  //枚举最高位的数
	{
		int now=a[i];			 //最高位的数字
		for(int j=(i==cnt);j<now;j++)//枚举次高位能填入的数
		 if(abs(j-last)>=2) res+=f[i][j];
		if(abs(now-last)<2) break; //最高位取不了now剪枝了直接
		last=now;
		if(i==1) res++;
	}
	//因为最高位要从1开始，所以后面不满cnt位的要另算
	for(int i=1;i<cnt;i++)
	 for(int j=1;j<=9;j++)
	  res+=f[i][j];
	return res;
